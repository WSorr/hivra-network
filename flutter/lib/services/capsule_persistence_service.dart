...
class CapsulePersistenceService {
    static final CapsulePersistenceService _instance = CapsulePersistenceService._internal();
    ...
    Future<void> clearPersistedData(HivraBindings hivra, {bool includeBackup = false}) async {
        final dir = await _currentCapsuleDir(hivra);
        final stateFile = File('${dir.path}/$_stateFileName');
        final ledgerFile = File('${dir.path}/$_ledgerFileName');
        final backupFile = File('${dir.path}/$_backupFileName');
    ...
    Future<String?> exportCapsuleBackupToPath(String pubKeyHex, String targetPath) async {
        final docs = await getApplicationDocumentsDirectory();
        final capsuleDir = Directory('${docs.path}/$_capsulesDirName/$pubKeyHex');
        final ledgerFile = File('${capsuleDir.path}/$_ledgerFileName');
    ...
    Future<bool> refreshCapsuleSnapshot(HivraBindings hivra, String pubKeyHex) async {
    ...
    Future<void> deleteCapsule(String pubKeyHex, {bool deleteLocalData = false}) async {
        if (deleteLocalData) {
            final docs = await getApplicationDocumentsDirectory();
            final capsuleDir = Directory('${docs.path}/$_capsulesDirName/$pubKeyHex');
            if (await capsuleDir.exists()) {
    ...
    Future<bool> hasStoredSeed(String pubKeyHex) async {
        String? encoded;
        try {
            encoded = await _secureStorage.read(key: '$_seedKeyPrefix$pubKeyHex');
        } catch (_) {
    ...
    Future<void> activateCapsule(HivraBindings hivra, String pubKeyHex) async {
        ...
        if (storedSeed != null) {
            ...
            final currentHex =
                currentPubKey != null && currentPubKey.length == 32 ? _bytesToHex(currentPubKey) : null;
    ...
    Future<Map<String, dynamic>?> _readStateForCurrentCapsule(HivraBindings hivra) async {
        final dir = await _currentCapsuleDir(hivra);
        final stateFile = File('${dir.path}/$_stateFileName');
    ...
    Future<Directory> _currentCapsuleDir(HivraBindings? hivra, {bool create = false}) async {
        final docs = await getApplicationDocumentsDirectory();
    ...
    Future<Directory> _capsuleDirForHex(String pubKeyHex, {bool create = false}) async {
        final docs = await getApplicationDocumentsDirectory();
        final capsulesRoot = Directory('${docs.path}/$_capsulesDirName');
        if (create && !await capsulesRoot.exists()) {
            await capsulesRoot.create(recursive: true);
    ...
    Future<void> _storeSeedForCapsule(String pubKeyHex, Uint8List seed) async {
        ...
        try {
            await _secureStorage.write(key: '$_seedKeyPrefix$pubKeyHex', value: encoded);
            return;
        } catch (_) {
    ...
    Future<Map<String, dynamic>?> _readStateForCapsuleHex(String pubKeyHex) async {
    ...
    CapsuleLedgerSummary _parseLedgerSummary(String json) {
        ...
        try {
            ...
            for (final eventRaw in events) {
                ...
                switch (kind) {
                    ...
                    case 5:
                        ...
                        if (starter != null) {
                            activeStartersById[_bytesToHex(Uint8List.fromList(starter.starterId))] =
                                starter.kindCode;
                        }
                        break;
                    case 6:
                        ...
                        if (burnedId != null) {
                            activeStartersById.remove(_bytesToHex(Uint8List.fromList(burnedId)));
                        }
                        break;
                    case 7:
            ...
            final relationshipCount = (relationshipEstablished - relationshipBroken).clamp(0, 9999);
    ...
    _BackupMeta? _extractBackupMeta(String rawJson) {
        try {
            ...
            return _BackupMeta(
                isGenesis: m['is_genesis'] == true ? true : (m['is_genesis'] == false ? false : null),
                isNeste: m['is_neste'] == true ? true : (m['is_neste'] == false ? false : null),
            );
        } catch (_) {
            return null;
        }
...
class CapsuleIndexEntry {
    ...
    static CapsuleIndexEntry fromMap(Map<String, dynamic> map) {
        final created = DateTime.tryParse(map['createdAt']?.toString() ?? '') ?? DateTime.now().toUtc();
        final last = DateTime.tryParse(map['lastActive']?.toString() ?? '') ?? created;
...
class _CapsulesIndex {
    ...
    static _CapsulesIndex empty() => _CapsulesIndex(activePubKeyHex: null, capsules: {});
